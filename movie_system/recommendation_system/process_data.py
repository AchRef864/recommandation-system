...
def prepare_data():
    ...
    df1 = pd.read_csv("d:/GitHub/recommandation-system/movie_data/movies.csv")
    # 'credits.csv' file contains cast, crew, and movie title
    df2 = pd.read_csv("d:/GitHub/recommandation-system/movie_data/credits.csv")
...
def preprocess_text(movie_list):
    ...
    row = [
        lemm.lemmatize(w)
        for w in words
        if not w in stopwords.words() and not len(w) == 1
    ]
    return str(row)
...
def all_info(movie_list):
    ...
    """
    return (
        " ".join(movie_list["cast"])
        + " "
        + movie_list["director"]
        + " "
        + " ".join(movie_list["genres"])
    )
